...
class DashboardController extends Controller {
	...
	public function index() {
		...
		if ($user_type == 'customer') {
			...
			// Calculate total loan statistics for admin (approved and completed loans)
			$loans = Loan::whereIn('status', [1, 2])->get();
			$data['total_capital'] = $loans->sum('applied_amount'); // Principal amount only
			$data['total_paid'] = 0;
			$data['total_balance'] = 0;
			$data['total_expected_interest'] = 0;  // Total interest to be earned
			$data['remaining_interest'] = 0;      // Interest yet to be collected

			foreach ($loans as $loan) {
				// Principal calculations
				$total_paid = $loan->payments()->sum('repayment_amount') - $loan->payments()->sum('interest');  // Principal payments only
				$data['total_paid'] += $total_paid;
				$data['total_balance'] += ($loan->applied_amount - $total_paid);
				
				// Interest calculations
				$total_expected_interest = $loan->repayments()->sum('interest'); // Total interest from repayment schedule
				$interest_paid = $loan->payments()->sum('interest'); // Interest already paid
				
				$data['total_expected_interest'] += $total_expected_interest;
				$data['remaining_interest'] += ($total_expected_interest - $interest_paid);
			}
		}
